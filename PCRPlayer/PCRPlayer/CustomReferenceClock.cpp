...
STDMETHODIMP CustomReferenceClock::NonDelegatingQueryInterface(
    REFIID riid,
...
CustomReferenceClock::~CustomReferenceClock()
{
#ifdef DXMPERF
    PERFLOG_DTOR( L"CustomReferenceClock", (IReferenceClock *) this );
#endif // DXMPERF
...
// of calling the schedulers Advise method.  (Refere to CustomReferenceClock::AdviseThread()
// to see what such a thread has to do.)
CustomReferenceClock::CustomReferenceClock( __in_opt LPCTSTR pName, 
...
, delay_(0)
{
...
#ifdef DXMPERF
    PERFLOG_CTOR( pName ? pName : L"CustomReferenceClock", (IReferenceClock *) this );
#endif // DXMPERF
    ...
    else
    {
        ...
        #ifdef PERF
            m_idGetSystemTime = MSR_REGISTER(TEXT("CustomReferenceClock::GetTime"));
        #endif
...
void CustomReferenceClock::Restart (IN REFERENCE_TIME rtMinTime)
{
    Lock();
...
STDMETHODIMP CustomReferenceClock::GetTime(__out REFERENCE_TIME *pTime)
{
    HRESULT hr;
    if (pTime)
    {
...
STDMETHODIMP CustomReferenceClock::AdviseTime(
...
STDMETHODIMP CustomReferenceClock::AdvisePeriodic(
...
STDMETHODIMP CustomReferenceClock::Unadvise(DWORD_PTR dwAdviseCookie)
{
    return m_pSchedule->Unadvise(dwAdviseCookie);
}


REFERENCE_TIME CustomReferenceClock::GetPrivateTime()
{
    ...
    DWORD dwTime = timeGetTime() + delay_;
    {
...
STDMETHODIMP CustomReferenceClock::SetTimeDelta(const REFERENCE_TIME & TimeDelta)
{
#ifdef DEBUG
...
DWORD __stdcall CustomReferenceClock::AdviseThreadFunction(__in LPVOID p)
{
    return DWORD(reinterpret_cast<CustomReferenceClock*>(p)->AdviseThread());
}

HRESULT CustomReferenceClock::AdviseThread()
{
...
HRESULT CustomReferenceClock::SetDefaultTimerResolution(
...
HRESULT CustomReferenceClock::GetDefaultTimerResolution(
...
void CustomReferenceClock::SetDelay(int time)
{
	CAutoLock cObjectLock(this);
	delay_ = time;
}
