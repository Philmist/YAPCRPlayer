...
void CPacketQueue::Add(CAutoPtr<Packet> p)
{
	...
	if(p)
	{
		...
		{
			Packet* tail = GetTail();			
			int oldsize = (int)tail->GetCount();
			int newsize = (int)(tail->GetCount() + p->GetCount());
...
void CPacketQueue::Forearch(REFERENCE_TIME time)
{
	CAutoLock cAutoLock(this);
...
bool CPacketQueue::GetHeadTime(REFERENCE_TIME& time)
{
	CAutoLock cAutoLock(this);
...
bool CPacketQueue::GetTailTime(REFERENCE_TIME& time)
{
	CAutoLock cAutoLock(this);
...
bool CPacketQueue::GetTotalTime(REFERENCE_TIME& time)
{
	CAutoLock cAutoLock(this);
	...
	if (head && tail)
	{
		time = tail->rtStart - head->rtStart;
		return true;
	}
	return false;
}
...
HRESULT CBaseSplitterOutputPin::SetName(LPCWSTR pName)
{
	...
	wcscpy_s(m_pName, lstrlen(pName), pName);
	return S_OK;
}
...
{
	for(int i = 0; i < (int)m_mts.GetCount(); i++)
	{
		if(*pmt == m_mts[i])
			return S_OK;
	}
...
HRESULT CBaseSplitterOutputPin::GetMediaType(int iPosition, CMediaType* pmt)
{
	...
	if(iPosition >= (int)m_mts.GetCount()) return VFW_S_NO_MORE_ITEMS;
...
void CBaseSplitterOutputPin::QueueForeach(REFERENCE_TIME time)
{
	m_queue.Forearch(time);
}

void CBaseSplitterOutputPin::QueueRemoveAll()
{
	m_queue.RemoveAll();
}
...
bool CBaseSplitterOutputPin::QueueHeadTime(REFERENCE_TIME& time)
{
	return m_queue.GetHeadTime(time);
}

bool CBaseSplitterOutputPin::QueueTailTime(REFERENCE_TIME& time)
{
	return m_queue.GetTailTime(time);
}

bool CBaseSplitterOutputPin::QueueTotalTime(REFERENCE_TIME& time)
{
	return m_queue.GetTotalTime(time);
}
...
bool CBaseSplitterOutputPin::IsActive()
{
	...
	CComPtr<IPin> pPin = this;
	CComPtr<IPin> pPinIn;
	if (SUCCEEDED(pPin->ConnectedTo(&pPinIn)) && pPinIn)
	{
		CComPtr<IBaseFilter> filter = GetFilterFromPin(pPinIn);
		...
		if (filter)
		{
			...
			CComPtr<IPin> pPinOut = GetFirstPin(filter, PINDIR_OUTPUT);
			if (pPinOut)
			{
				CComPtr<IPin> pPinTo;
				if (SUCCEEDED(pPinOut->ConnectedTo(&pPinTo)))
				{
					return true;
				}
			}
...
*/
	...
	do
	{
		...
		if(nBytes > pSample->GetSize())
		{
			...
			props.cbBuffer = (long)(nBytes*3/2);
...
*/
		...
		if(S_OK != (hr = pSample->SetActualDataLength((long)nBytes))) break;
...
DWORD CBaseSplitterFilter::ThreadProc()
{
	...
	for(DWORD cmd = (DWORD)-1; ; cmd = GetRequest())
	{
		if(cmd == CMD_EXIT)
		{
			m_hThread = NULL;
			Reply(S_OK);
			//DebugText(L"ThreadProc CMD_EXIT\n");
			return 0;
		}
...
STDMETHODIMP CBaseSplitterFilter::GetCurFile(LPOLESTR* ppszFileName, AM_MEDIA_TYPE* pmt)
{
	...
	*ppszFileName = (LPOLESTR)CoTaskMemAlloc((m_fn.GetLength()+1)*sizeof(WCHAR));
	...
	wcscpy_s(*ppszFileName, m_fn.GetLength(), m_fn);
	return S_OK;
}
...
{
	return E_NOTIMPL;//ChapGet((int)MarkerNum-1, NULL, pbstrMarkerName);
}
