...
.DESCRIPTION
    zhongfly/mpv-winbuild の「ピン留めしたリリース」から GPL ビルドの mpv-dev-x86_64-*.7z を
    取得し、third_party/mpv-dev/ へ展開する。展開後、mpv.def から lib.exe で mpv.lib
    （MSVC import lib）を生成する。
    生成物:
      ...
      third_party/mpv-dev/PROVENANCE.txt         … 取得したバイナリの同定情報（GPLv3 §6 用）

    冪等。既に client.h と mpv.lib があれば何もしない（-Force で再取得）。

    GPLv3 §6 コンプライアンス: 配布する libmpv-2.dll は下記の $PinnedTag / $ExpectedSha256 に
    ピン留めされ、CORRESPONDING-SOURCE.md・THIRD-PARTY-NOTICES.md・scripts/mirror-mpv-source.ps1 の
    記載と一致する。-Latest で最新へ更新した場合はそれら 3 つも必ず追従させること
    （DLL と対応ソース/ライセンス記載がずれると §6 違反になる）。
...
param(
    [switch]$Force,
    # ピンを無視して最新リリースを取得する（ライセンス記載の追従が必要）。
    [switch]$Latest
)
...
# ---- ピン留め（CORRESPONDING-SOURCE.md と一致させること） ----
$PinnedTag      = '2026-06-08-6444c05059'
$ExpectedSha256 = '0441491A7E275CC00570EDDE5B353A166866B5D80B0C986C60FBDAE6D8669B7E'
...
# ---- リリースの mpv-dev-x86_64（GPL/汎用）資産を解決 ----
$headers = @{ 'User-Agent' = 'YAPCRPlayer-fetch-mpv-dev' }
if ($env:GITHUB_TOKEN) { $headers['Authorization'] = "Bearer $env:GITHUB_TOKEN" }
if ($Latest) {
    Write-Step '最新リリース情報を取得中 (zhongfly/mpv-winbuild)  ※ピン無視'
    Write-Warning 'ピンを無視して最新を取得します。CORRESPONDING-SOURCE.md / THIRD-PARTY-NOTICES.md / mirror-mpv-source.ps1 / licenses/ を必ず追従させてください。'
    $release = Invoke-RestMethod -Headers $headers `
        -Uri 'https://api.github.com/repos/zhongfly/mpv-winbuild/releases/latest'
} else {
    Write-Step "ピン留めリリースを取得中 (zhongfly/mpv-winbuild @ $PinnedTag)"
    $release = Invoke-RestMethod -Headers $headers `
        -Uri "https://api.github.com/repos/zhongfly/mpv-winbuild/releases/tags/$PinnedTag"
}
...
# ---- SHA-256 検証（ピン時のみ。改ざん/取り違え防止 ＋ 対応ソースとの一致保証） ----
$actualSha = (Get-FileHash $archive -Algorithm SHA256).Hash
if (-not $Latest) {
    if ($actualSha -ne $ExpectedSha256) {
        throw "SHA-256 不一致。期待=$ExpectedSha256 実際=$actualSha （書庫破損か、ピン値の更新漏れ）"
    }
    Write-Step "SHA-256 検証 OK"
} else {
    Write-Step "SHA-256 (latest, 検証スキップ): $actualSha"
}
...
# ---- PROVENANCE 記録（GPLv3 §6 — このバイナリが何であるかを残す） ----
$mpvCommit = if ($release.body -match 'commit/([0-9a-f]{40})') { $Matches[1] } else { '(不明)' }
$provenance = @"
# libmpv-2.dll provenance — generated by fetch-mpv-dev.ps1
# 配布する libmpv-2.dll の同定情報。対応ソースは CORRESPONDING-SOURCE.md / mirror-mpv-source.ps1。
fetched-at    : $(Get-Date -Format o)
source-repo   : https://github.com/zhongfly/mpv-winbuild
release-tag   : $($release.tag_name)
asset         : $($asset.name)
asset-sha256  : $actualSha
mpv-commit    : $mpvCommit
pinned        : $(if ($Latest) { 'no (-Latest)' } else { "yes ($PinnedTag)" })
"@
Set-Content -Path (Join-Path $DevDir 'PROVENANCE.txt') -Value $provenance -Encoding utf8

Write-Step '完了'
Write-Host "  ヘッダ      : $Header"
Write-Host "  import lib  : $ImportLib"
Write-Host "  ランタイム  : $(Join-Path $DevDir 'libmpv-2.dll')"
Write-Host "  provenance  : $(Join-Path $DevDir 'PROVENANCE.txt')"
